...
func (e *Executor) prepareWebsocketHeaders(accessToken, accountID, sessionID, apiKey string, deviceCfg *proxy.DeviceProfileConfig, ginHeaders http.Header) http.Header {
	...
	if configuredProfile, ok := proxy.ConfiguredClientProfile(deviceCfg); ok {
		headers.Set("User-Agent", configuredProfile.UserAgent)
		if configuredProfile.Version != "" {
			headers.Set("Version", configuredProfile.Version)
		}
	} else if proxy.IsDeviceProfileStabilizationEnabled(deviceCfg) {
